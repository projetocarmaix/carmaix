package br.com.carmaix.domain;

import android.widget.TextView;

import java.util.Date;

/**
 * Created by fernando on 20/05/16.
 */
public class Avaliacao {
    Integer id;
    String nome;
    String avaliacaoId;
    String avaliacaoData;
    String avaliacaoMarca;
    String avaliacaoModelo;
    String avaliacaoPlaca;
    String avaliacaoAno;
    String avaliacaoClasse;
    String avaliacaoAvaliacao;
    String avaliacaoOpc;
    String avaliacaoNome;

    public Integer getId() {
        return id;
    }

    public void setId(Integer id) {
        this.id = id;
    }

    public String getNome() {
        return nome;
    }

    public void setNome(String nome) {
        this.nome = nome;
    }

    public String getAvaliacaoId() {
        return avaliacaoId;
    }

    public void setAvaliacaoId(String avaliacaoId) {
        this.avaliacaoId = avaliacaoId;
    }

    public String getAvaliacaoData() {
        return avaliacaoData;
    }

    public void setAvaliacaoData(String avaliacaoData) {
        this.avaliacaoData = avaliacaoData;
    }

    public String getAvaliacaoMarca() {
        return avaliacaoMarca;
    }

    public void setAvaliacaoMarca(String avaliacaoMarca) {
        this.avaliacaoMarca = avaliacaoMarca;
    }

    public String getAvaliacaoModelo() {
        return avaliacaoModelo;
    }

    public void setAvaliacaoModelo(String avaliacaoModelo) {
        this.avaliacaoModelo = avaliacaoModelo;
    }

    public String getAvaliacaoPlaca() {
        return avaliacaoPlaca;
    }

    public void setAvaliacaoPlaca(String avaliacaoPlaca) {
        this.avaliacaoPlaca = avaliacaoPlaca;
    }

    public String getAvaliacaoAno() {
        return avaliacaoAno;
    }

    public void setAvaliacaoAno(String avaliacaoAno) {
        this.avaliacaoAno = avaliacaoAno;
    }

    public String getAvaliacaoClasse() {
        return avaliacaoClasse;
    }

    public void setAvaliacaoClasse(String avaliacaoClasse) {
        this.avaliacaoClasse = avaliacaoClasse;
    }

    public String getAvaliacaoAvaliacao() {
        return avaliacaoAvaliacao;
    }

    public void setAvaliacaoAvaliacao(String avaliacaoAvaliacao) {
        this.avaliacaoAvaliacao = avaliacaoAvaliacao;
    }

    public String getAvaliacaoOpc() {
        return avaliacaoOpc;
    }

    public void setAvaliacaoOpc(String avaliacaoOpc) {
        this.avaliacaoOpc = avaliacaoOpc;
    }

    public String getAvaliacaoNome() {
        return avaliacaoNome;
    }

    public void setAvaliacaoNome(String avaliacaoNome) {
        this.avaliacaoNome = avaliacaoNome;
    }

}
